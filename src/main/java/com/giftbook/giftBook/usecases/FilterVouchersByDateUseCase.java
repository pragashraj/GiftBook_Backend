...
public class FilterVouchersByDateUseCase {
    private static final Logger log = LoggerFactory.getLogger(FilterVouchersByDateUseCase.class);
    ...
    public FilterVouchersByDateUseCase(VoucherRepository voucherRepository,
    ...
    public PageableCoreVoucher execute() throws EntityNotFoundException {
        ...
        Page<Voucher> voucherPage = voucherRepository.findAllByUserAndCreatedAtGreaterThanEqualAndCreatedAtLessThanEqualOrderByCreatedAt(
                user, parseDate(start), parseDate(end), PageRequest.of(page, 10)
        );
